...
onMounted(async () => {
  try {
    ...
    if (docSnap.exists()) {
      propertyAddress.value = docSnap.data().address
  ...
  <v-container class="py-8">
    <v-card class="mx-auto" max-width="1000" elevation="2">
      ...
      <v-card-text>
        ...
        <v-row>
          <v-col v-for="(rec, index) in propertyData.recommendations" :key="index" cols="12" md="4">
        ...
        <v-row>
          <v-col cols="12" sm="6">
            <v-card variant="outlined">
              ...
              <v-card-text class="text-h5">${{ propertyData.annualUtilitySpend }}</v-card-text>
            </v-card>
          </v-col>
          <v-col cols="12" sm="6">
            <v-card variant="outlined">
              <v-card-title>Annual Savings Potential</v-card-title>
              <v-card-text class="text-h5">${{ propertyData.savingsPotential }}</v-card-text>
            </v-card>
          </v-col>
        </v-row>
      </v-card-text>
